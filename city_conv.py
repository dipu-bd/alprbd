...
# Network Parameters
n_input = 4*28*28 # MNIST data input (img shape: 28*28)
n_classes = len(cfg.CITIES) # MNIST total classes (0-9 digits)
...
def conv_net(x, weights, biases, dropout):
    ...
    x = tf.reshape(x, shape=[-1, 28, 4*28, 1])
...
with tf.Session() as sess:
    ...
    # test letters
    letters = cfg.CITIES
    for file in read_images(cfg.CITY_SAMPLES):
        # prepare image data
